...
| Reminders | Basic loop | Smart reminders with overdue re-notification + priority display + throttled sends |
...
| Error handling | Ad-hoc | Structured logging + global `on_app_command_error` + per-View `on_error` handlers |
| Config | Scattered | Single typed `AppConfig` from `.env` |
| Task views | Basic | Interactive UI modals, paginated list, category select dropdown, priority dropdown, pin toggle |
| Daily digest | None | Configurable daily summary (optimised: single merged query per user) |
...
| Persistent views | None | Buttons on old messages survive bot restarts (with full Category dropdown restored) |
| Interaction safety | Basic | All buttons/dropdowns defer before DB operations — zero 3-second timeout errors |
...
│                            # on_interaction: INSERT OR IGNORE + UPDATE last_active (safe for new users)
...
│   │                        # BulkWriter (async write-batching, uses get_running_loop()),
│   │                        # WAL checkpoint, backup, exponential backoff retry
...
│   │                        # All commands defer() before DB operations
│   ├── settings_cog.py      # /setup /lang /help /category /admin
│   ├── reminders_cog.py     # Background loops: reminder (throttled 0.3s/msg), recurring,
│   │                        # backup, cache-cleanup, daily-digest (merged query), WAL-checkpoint,
│   │                        # deadline-dm
│   └── task_views.py        # Discord UI: AddTaskModal, EditTaskModal, TaskActionView,
│                            # TaskListView, DeleteConfirmView, LanguageView,
│                            # CategorySelect, PrioritySelectView, PriorityEditSelect
│                            # All callbacks defer() + on_error() for graceful failure handling
│
├── utils/
...
│
├── locales/
│   ├── i18n.py              # Translation engine (lazy-loads locale modules, falls back to Thai)
│   ├── th.py                # Thai strings
...
| `/delete [id]` | 🗑️ Delete a task (with confirmation dialog) |
...
## 🖱️ Interactive Task View (TaskActionView)

When you create or view a task, an interactive panel is attached with:

| Row | Controls |
|---|---|
| Row 0 | ✅ Done · ✏️ Edit · 📌 Pin/Unpin |
| Row 1 | 🗑️ Delete · 💤 Snooze +1d |
| Row 2 | ➕ Add Subtask |
| Row 3 | 🏷️ Category Select dropdown |
| Row 4 | ⚡ Priority Select dropdown |

All buttons and dropdowns:
- **Defer immediately** before any database operation — eliminating Discord's 3-second timeout error
- **Persist across restarts** — registered via `register_all_persistent_views()` on every bot startup (including Category dropdown)
- **Report errors gracefully** — each View has an `on_error()` handler that sends an ephemeral error message instead of silently failing
...
- **Safe user creation** — `on_interaction` uses `INSERT OR IGNORE` before `UPDATE last_active` so users who haven't run `/setup` can't cause silent DB errors
...
| `DB_POOL_SIZE` | `10` | Connection pool size (code default); `.env.example` ships `5` |
...
|---|---|---|
| `reminder_loop` | Configurable (default 30 min) | Send deadline reminders and overdue alerts to user notification channels; throttled 0.3 s between sends to avoid Discord rate limits |
| `recurring_loop` | Configurable (default 60 min) | Renew completed recurring tasks by creating the next occurrence |
| `daily_digest_loop` | Every 5 min (fires once/day at configured UTC hour) | Send a daily summary of today's tasks; overdue count computed via single merged SQL subquery (no N+1) |
...
- **BulkWriter** — accumulates write operations (audit log, reminder timestamps, dm_reminded updates) and flushes them in a single transaction every 500 ms, dramatically reducing per-row transaction overhead. Uses `asyncio.get_running_loop()` (Python 3.10+ safe)
...
- **Exponential backoff** — DB write retries use `rand(0, base × 2^attempt)` jitter to avoid thundering herd on lock contention
...
### Interaction Safety
All Discord button and dropdown callbacks follow this pattern:
1. `await interaction.response.defer(ephemeral=True)` — called immediately as the first line
2. Database operation (via `asyncio.to_thread` — non-blocking)
3. `await interaction.followup.send(...)` — used for all responses after defer

Each View class (`TaskActionView`, `DeleteConfirmView`, `PrioritySelectView`) implements `on_error()` to catch any unhandled exception and reply with a user-friendly error message instead of silently failing.

### Persistent Views (Post-Restart Safety)
On every bot startup, `register_all_persistent_views()` is called from `setup_hook`. It:
1. Fetches all tasks (up to 5,000) with a single JOIN query
2. Pre-fetches **all categories in one query** and groups them by owner (no N+1)
3. Registers a `TaskActionView` (with Category dropdown) for every task using `bot.add_view()`

This ensures buttons and dropdowns on **all existing messages** remain interactive after a bot restart — no new tasks required.
...
### Supported Date Formats (Deadline Input)
| Format | Example |
|---|---|
| `DD/MM/YYYY HH:MM` | `31/12/2026 23:59` |
| `DD-MM-YYYY HH:MM` | `31-12-2026 23:59` |
| `YYYY-MM-DD HH:MM` | `2026-12-31 23:59` |
| `DD/MM/YYYY` | `31/12/2026` *(defaults to 23:59)* |
| `DD-MM-YYYY` | `31-12-2026` *(defaults to 23:59)* |
| `YYYY-MM-DD` | `2026-12-31` *(defaults to 23:59)* |
